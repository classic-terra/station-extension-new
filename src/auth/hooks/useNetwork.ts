...
export const useNetworkOptions = () => {
  return [
    ...
    { value: "classicv2test", label: "Classic v2 TestNet" },
...
export const useNetwork = (): Record<ChainID, InterchainNetwork> => {
  ...
  if (connectedWallet.status === WalletStatus.WALLET_CONNECTED) {
    if (network !== "mainnet" && "phoenix-1" in connectedWallet.network) {
      ...
      setNetwork("classic")
    } else if (network !== "classic" && "rebel-2" in connectedWallet.network) {
      setNetwork("classictestnet")
    } else if (network !== "classic" && "titan-1" in connectedWallet.network) {
      setNetwork("classicv2test")
    } else if (
      network !== "localterra" &&
